...
public class PublicacaoAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup viewGroup) {
        ...
        int tela = position%2 == 0? R.layout.publicacao_linha_par : R.layout.publicacao_linha_impar;

        if (convertView == null){
            linha = LayoutInflater.from(context).inflate(tela, viewGroup, false);
        ...
        Picasso.with(this.context).load(publicacao.getFoto()).fit().into(holder.foto, new VericadorDeRetorno(holder.progress ,holder.foto));
    ...
    class VericadorDeRetorno implements Callback{
        private ProgressBar holder;
        private ImageView foto;

        public VericadorDeRetorno(ProgressBar holder,ImageView foto){
            this.holder = holder;
            this.foto = foto;
        }
        ...
        public void onError() {
            holder.setVisibility(View.GONE);
            foto.setImageDrawable(context.getResources().getDrawable(R.drawable.ic_car));        }
